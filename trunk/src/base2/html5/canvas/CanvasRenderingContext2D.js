...
var CanvasRenderingContext2D = Base.extend({
	...
	getCoords: function(x,y) {
		...
		coord.toString = function() { return Math.round(this.x) + ',' + Math.round(this.y) };
		return coord;
	}
...
CanvasRenderingContext2D.implement({
	...
	*/
	arc: function(x, y, rad, a0, a1, cw) {
		...
		var x0 = Math.cos(a0) * rad, y0 = Math.sin(a0) * rad,
			x1 = Math.cos(a1) * rad, y1 = Math.sin(a1) * rad;
		...
		this.path[this.l++] = [
			cw ? 'at ' : 'wa ',
			Math.round(c.x - aSXr) + ',' + Math.round(c.y - aSYr), ' ',
			Math.round(c.x + aSXr) + ',' + Math.round(c.y + aSYr), ' ',
	...
	*/
	processColorObject: function(obj) {
		...
		if(obj.addColorStop) {
			...
			if(obj.stops) for (var i = 0, j = obj.stops.length; i < j; i++) stops += Math.round(100 * obj.stops[i][0]) + '% ' + obj.stops[i][1];
			ret += ((obj.r0) ?
...
CanvasRenderingContext2D.implement({
	...
	*/
	...
	dotmult: function(x,y) {
		return map(y, function(val) { return x * val; });
	},
	...
	*/
	drawImage: function (image) {
		...
		var syh = sy / h, sxw = sx / w,
			m = this.m,
			Z = this.Z,
			d = map(this.getCoords(dx, dy), function(val) { return Math.round(val / Z); });
...
*/
var CanvasGradient = Base.extend({

	constructor: function(x0, y0, x1, y1, ctx) {
...
*/
var CanvasPattern = Base.extend({

	constructor: function(img, rep) {
		this.img = img;
		this.rep = rep;
	}
