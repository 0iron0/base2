...
new function() {
	...
	// fix enumeration for Safari 1.2/3
  	var C = function(){this.i=1}, c = 0;
  	C.prototype = {i:1};
	for (var i in new C) c++;
	if (c > 1) {
		$Legacy.forEach = function(fn, object, block, context) {
	...
	try {
		...
		$Legacy.exists = function(k) {
			for (var i in this["#keys"]) if (i == k) return true;
			return false;
		};
	}
